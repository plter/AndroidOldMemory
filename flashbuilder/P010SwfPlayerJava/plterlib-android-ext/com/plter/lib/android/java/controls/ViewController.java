 ...
 *
 */
public class ViewController extends EventTarget implements IViewController{
	...
	public void popViewController(boolean animated){
		if (next!=null) {
			if (next.on().backing.dispatch(new ViewControllerEvent(ViewControllerEvent.ON_BACKING,true))) {
	...
	private void removeNext(){
		...
		next.on().back.dispatch(new ViewControllerEvent(ViewControllerEvent.ON_BACK));
	...
	public ViewControllerEvents on() {
		return evts;
	}


	private final ViewControllerEvents evts=new ViewControllerEvents();

	public class ViewControllerEvents extends Events{
